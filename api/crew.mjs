...
if(['settle','allocate','receipt','assignRole','resolveCorrection','resetAll','saveMember'].includes(command.action)&&state.roles[u.id]!=='master')throw Error('Nur Master darf diese Funktion nutzen.');if(['drink','archiveDrink','order','done','syncEvents','decideApplication'].includes(command.action)&&!['admin','master'].includes(state.roles[u.id]))throw Error('Nur Admin oder Master.');
...
const out=await run(state,u.id,c);out.state.requests??=state.requests||{};out.state.requests[key]={fingerprint,result:out.body};if(await commit(revision,out.state))return json(res,200,out.body)}
